    ...
    return (
        <main className="center m-screen mx-5 my-5">
            <div className="flex flex-col bg-gray-200 p-4 border-1 border-blue-900 rounded text-blue-600 shadow-lg w-96 p-5 gap-2">
                ...
                <img src={item.image} className="border-gray-900 border-2"/>
                <div className="text-black h-24 overflow-y-hidden">{item.description}</div>
                <section className="flex flex-row flex-wrap">
                <h3 className="py-3 pr-3 flex-auto font-extrabold text-lg">${item.price}</h3>
                    <button className="bg-blue-700 text-white uppercase py-2 px-2 rounded-full flex-auto">Add to Cart</button>
                </section>   
            </div>
      </main>
    )
